...
all: $(LIBPDS) $(LIBPDS_STATIC) $(LIBPD) vanilla_install externals_list.txt
...
externals_list.txt: make.scm
	bash -c "if ! ./make.scm print-external-names >externals_list.txt ; then rm -f externals_list.txt ; echo 'could not make externals_list.txt' ; failed-failed-failed ; fi "
	bash -c "if [[ ! -s externals_list.txt ]] ; then rm -f externals_list.txt ; echo 'could not make externals_list.txt' ; failed-failed-failed ; fi "
